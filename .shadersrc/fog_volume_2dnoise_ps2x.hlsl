...
float4 main(PS_INPUT i) : COLOR
{
    ...
    float3 localPosAbs = abs(midPoint - BoxOrigin); // keep for edge fade
    float3 localPos = midPoint - BoxOrigin;
    ...
    float noiseVal = tex2D(NoiseSampler, noiseUV).r; // sample red channel
    noiseVal = clamp( noiseVal, NoiseMinInfluence, NoiseMaxInfluence );

    // Rescale density for more intuitive artistic control
    float scaledDensity = FogDensity * 0.02;
    float fogFactor = 1.0 - exp(-scaledDensity * segmentLen * noiseVal);
